...
export function resolveInputCapture(input) {
...
export function readManifest(captureDir) {
...
export function writeManifest(captureDir, manifest) {
    writeFileSync(join(captureDir, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`);
}
export function updateManifest(captureDir, update) {
...
export function readLatest() {
    if (!existsSync(latestPath))
        throw new Error(`No latest capture pointer found at ${latestPath}`);
    return JSON.parse(readFileSync(latestPath, "utf8"));
}
export function writeLatest(captureDir, manifest) {
    mkdirSync(atscHome, { recursive: true });
    const latest = {
...
export function extractAudio(input) {
    ...
    if (!existsSync(audioPath)) {
        ...
        if (commandExists("ffmpeg")) {
            ...
            if (result.status !== 0)
                errors.push(`ffmpeg m4a fallback failed: ${trimToolError(result.stderr)}`);
        }
        else if (commandExists("afconvert")) {
            const result = run("afconvert", [recordingPath, audioPath, "-f", "m4af", "-d", "aac"]);
            if (result.status !== 0)
                errors.push(`afconvert m4a fallback failed: ${trimToolError(result.stderr)}`);
        }
    }
    const info = getMediaInfo(recordingPath);
...
export function transcribe(input, model = "base.en") {
...
export function setupTranscriber() {
    if (!commandExists("python3"))
        throw new Error("python3 is required to create the transcriber virtualenv.");
    mkdirSync(atscHome, { recursive: true });
    const venvDir = join(atscHome, ".venv");
    if (!existsSync(venvDir)) {
        const created = run("python3", ["-m", "venv", venvDir]);
        if (created.status !== 0)
            throw new Error(`Failed to create .venv: ${trimToolError(created.stderr)}`);
    }
    ...
    if (upgraded.status !== 0)
        throw new Error(`Failed to upgrade pip: ${trimToolError(upgraded.stderr)}`);
    const installed = run(pip[0], [...pip.slice(1), "install", "faster-whisper"]);
    if (installed.status !== 0)
        throw new Error(`Failed to install faster-whisper: ${trimToolError(installed.stderr)}`);
}
...
export function getLatestCaptureDir() {
    if (existsSync(latestPath))
        return readLatest().captureDir;
...
export function doctor() {
    ...
    const target = targetResult.status === 0 && targetResult.stdout.trim()
        ...
        : undefined;
    if (!saveLocation)
        errors.push("Screenshot save location is not configured; set Screenshot Options to a folder.");
    else if (!existsSync(saveLocation))
        errors.push(`Screenshot save location does not exist: ${saveLocation}`);
    if (target && target !== "file")
        errors.push(`Screenshot target is "${target}", not "file"; set Screenshot Options to save to a folder.`);
    const transcriberInstalled = existsSync(join(atscHome, ".venv", "bin", "python"));
    if (!transcriberInstalled)
        errors.push("Transcriber virtualenv is missing; run: atsc setup-transcriber");
    let latestCapture;
    if (existsSync(latestPath)) {
        try {
...
export function getMediaInfo(recordingPath) {
    const errors = [];
    const fallbacks = [];
...
async function waitForNewStableMov(dir, before, startedAt) {
    ...
    while (Date.now() < deadline) {
        ...
        for (const candidate of candidates) {
            if (await isStableFile(candidate))
                return candidate;
        }
        await sleep(1000);
    }
    throw new Error(`Timed out waiting for a new .mov from Screenshot.app in ${dir}. ` +
...
async function isStableFile(path) {
...
export function resolveCaptureDir(input) {
...
function resolveCaptureDirOrPath(input) {
    const candidate = resolveHome(input);
    ...
    if (existsSync(byIdRecording))
        return byIdRecording;
    return candidate;
}
function trimToolError(value) {
